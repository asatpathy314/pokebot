...
class PokeAPIClient:
    ...
    def sanitize_message(message_array: list):
        original_message = (' '.join(message_array[1:])).lower()
        raw_message = ('-'.join(message_array[1:])).lower()
        query = ''.join(filter(lambda x: x.isalnum() or x == '-', raw_message))
        return original_message, query

    @staticmethod
    def get_pokemon(message_array: list):
        original_pokemon_name, pokemon_name = PokeAPIClient.sanitize_message(message_array)
        response = requests.get(f"https://pokeapi.co/api/v2/pokemon/{pokemon_name}")
        if response.status_code == 200:
            data = response.json()
            return data, original_pokemon_name, 0
        elif response.status_code == 404:
            return f'{original_pokemon_name} is not a Pokémon, please try again.', original_pokemon_name, -1
        else:
            return f'An error occurred, please try again.', original_pokemon_name, -2
    ...
    def get_type_multiplier(self, message_array: list, relation: str, is_move: bool = False):
        ...
        if is_move:
            ...
            data_response, original_pokemon_name, return_code = self.get_pokemon(message_array)
            if return_code < 0:
                return data_response, original_pokemon_name, return_code, return_code
            types_response = [type_info['type']['name'] for type_info in data_response['types']]
            ...
            return new_state, original_pokemon_name, data_response['sprites']['front_default'], 0

    @staticmethod
    def filter_weaknesses(weaknesses: dict):
        return ({k: v for k, v in weaknesses.items() if v == 4.00},
                {k: v for k, v in weaknesses.items() if v == 2.00},
                {k: v for k, v in weaknesses.items() if v == 1.00},
                {k: v for k, v in weaknesses.items() if v == 0.50},
                {k: v for k, v in weaknesses.items() if v == 0.25})
